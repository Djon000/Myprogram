...
def run_genetic_algorithm1(items, V, W, population_size, generations, mutation_rate, value_count):
    ...
    # Формуємо список вибраних предметів у форматі:
    # [name, volume, weight, value, item_index, копія_предмета]
    selected_items = []
    item_counts = {}
    ...
    for item_index in best_chromosome_ever:
        ...
        selected_items.append([name, volume, weight, value, item_index, count])
        item_counts[item_index] = count + 1
